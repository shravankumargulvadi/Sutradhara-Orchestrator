...
|---------|-----------|----------|-------------|
| Task rejected | `TaskAck(REJECTED)` | Reassign to next-best robot | `TASK_REJECTED` |
| Task failed | `TaskUpdate(FAILED)` | Replan affected tasks (Gemini call) | `REPLAN_TRIGGERED` |
| Robot offline | No `RobotState` for N seconds | Reassign ALL tasks from dead robot | `ROBOT_FAILURE_DETECTED` |
...
| Token budget exhausted | `TokenBudgetExhausted` | Mission → FAILED, in-flight tasks continue | `TOKEN_BUDGET_EXHAUSTED` |
